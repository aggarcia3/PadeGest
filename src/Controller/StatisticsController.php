...
class StatisticsController extends AppController
{
    ...
    public function index()
    {
        ...
        } elseif ($fechaInicio->month > $fechaFin->month) {
        ...
        foreach ($horas as $hora) :
            $reservas = TableRegistry::getTableLocator()->get('Reserva');
            $resultsIteratorObject4 = $reservas->find('all', ['conditions' => ['RESERVA.fechaInicio LIKE' => '%' . $hora . '%']]);
            ;
            $contadorAux = 0;
            $pistaId = -1;
        ...
        } elseif ($fechaInicio2->month > $fechaFin2->month) {
        ...
        for ($i2 = 0; $i2 < $mesesDiferencia2; $i2++) {
            $pagosPorMes = $pagos->find('all', ['conditions' => ['PAGO.fecha BETWEEN ' . '\'' . $fechaInicio2->i18nFormat('yyyy-MM-dd HH:mm:ss') . '\'' . ' AND ' . '\'' . $fechaInicio2->addMonth(1)->i18nFormat('yyyy-MM-dd HH:mm:ss') . '\'']]);
        ...
        $this->set(['contadores' => $contadores, 'contadorPistaReservas' => $contadorPistaReservas, 'pistaFinal' => $pistaFinal,  'contadorHoraReservas' => $contadorHoraReservas, 'horaFinal' => $horaFinal,
